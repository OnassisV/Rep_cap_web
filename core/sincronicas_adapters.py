...
# ---------------------------------------------------------------------------
# Auto-importar sincronicas de oferta_formativa_difoca a Capacitacion ORM
# ---------------------------------------------------------------------------

def sincronizar_capacitaciones_desde_oferta() -> int:
    """Crea registros Capacitacion para sincronicas que existen en
    oferta_formativa_difoca pero aun no tienen registro ORM.  Retorna
    la cantidad de registros nuevos creados."""
    ...
    try:
        ...
        logger.exception("Error leyendo oferta para sincronizar")
        return 0

    sincronicas = [
        r for r in rows
        if "sincron" in str(r.get("tipo_proceso_formativo", "")).lower()
    ]
    if not sincronicas:
        return 0

    existing_codigos = set(
        Capacitacion.objects.filter(cap_tipo="Capacitación sincrónica")
        .values_list("cap_codigo", flat=True)
    )

    created = 0
    for row in sincronicas:
        codigo = str(row.get("codigo", "")).strip()
        if not codigo or codigo in existing_codigos:
            continue
        try:
            anio_val = int(str(row.get("anio", "")).strip())
        except (ValueError, TypeError):
            anio_val = datetime.now().year

        Capacitacion.objects.create(
            cap_nombre=(
                str(row.get("denominacion_proceso_formativo", "")).strip()
                or f"Sincrónica {codigo}"
            ),
            cap_codigo=codigo,
            cap_tipo="Capacitación sincrónica",
            cap_estrategia=str(row.get("tipo_proceso_formativo", "")).strip(),
            cap_anio=anio_val,
            cap_estado="Borrador",
            creado_por=str(row.get("especialista_cargo", "")).strip() or "sistema",
            creado_nombre=str(row.get("especialista_cargo", "")).strip() or "Sistema",
        )
