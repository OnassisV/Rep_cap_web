...
def obtener_conexion_aula():
    """
    Obtiene conexión a BD de Aula Virtual (Chamilo).
    ...
    if hasattr(settings, 'AULA_DB'):
        ...
        return pymysql.connect(
            host=settings.AULA_DB.get('host'),
            user=settings.AULA_DB.get('user'),
            password=settings.AULA_DB.get('password'),
            database=settings.AULA_DB.get('database'),
            port=settings.AULA_DB.get('port', 3306),
            charset='utf8mb4'
        )
